...
/**
/**
 * 六、新增供應商列 (對齊 7 欄網格與刪除圖示)
 */
function addQuoteSupplierRow(data = {}) {
    ...
    // 正確對齊 7 欄網格結構：供應商名稱、施作項目、未稅價格、報價金額、簽約金額、稅%、刪除按鈕
    const html = `
        <div id="${rowId}" class="grid grid-cols-[1fr_1fr_1fr_1fr_1fr_1fr_32px] gap-3 items-center bg-stone-50 p-3 rounded-xl border border-stone-200">
            ...
            <div>
                <input type="number" value="${data.untaxed || ''}" placeholder="未稅價格" oninput="calculateQuoteTotals()" class="supplier-untaxed w-full bg-white border border-stone-300 rounded-xl px-3 py-1.5 text-xs focus:outline-none focus:border-[#C59B63] font-mono">
            </div>
            <div>
                <input type="number" value="${data.amount || ''}" placeholder="報價金額" oninput="calculateQuoteTotals()" class="supplier-amount w-full bg-white border border-stone-300 rounded-xl px-3 py-1.5 text-xs focus:outline-none focus:border-[#C59B63] font-mono">
            </div>
            <div>
                <input type="number" value="${data.contract || ''}" placeholder="簽約金額" oninput="calculateQuoteTotals()" class="supplier-contract w-full bg-white border border-stone-300 rounded-xl px-3 py-1.5 text-xs focus:outline-none focus:border-[#C59B63] font-mono text-rose-600 font-bold">
            </div>
            <div class="flex items-center space-x-1.5">
                <input type="number" value="${data.tax !== undefined ? data.tax : 5}" oninput="calculateQuoteTotals()" class="supplier-tax w-14 bg-white border border-stone-300 rounded-xl px-2 py-1.5 text-xs text-center font-mono">
                <span class="text-[10px] text-stone-500 font-bold">稅%</span>
            </div>
            <div class="flex justify-center">
                <button type="button" onclick="document.getElementById('${rowId}').remove(); calculateQuoteTotals();" class="text-stone-400 hover:text-rose-600 cursor-pointer transition">
    ...
    } else if (typeof window.calculateQuoteTotals === 'function') {
        window.calculateQuoteTotals();
    }
}

// 💡 雙重掛載，確保 HTML onclick 能順利呼叫
window.addQuoteSupplierRow = addQuoteSupplierRow;
